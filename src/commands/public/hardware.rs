...
pub async fn hardware(
    ...
    let servers: Servers = serde_json::from_str(&hardware_file).map_err(|e| format!("Failed to parse hardware file: {}", e))?;
    let mut embed = embed(&ctx).await?
        .title("Hypnos Server Hardware");
    for server in &servers.servers {
        embed = embed.field(&server.name, "", false)
